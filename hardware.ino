...
void getdata(char * tp, byte *nd, unsigned int length)
{
 ...
 if(topic == "unlock")
 {
    ...
    {
     // TempControl.publish("unlock", "Wrong pass");
      pass = 2;
    }
 }
}
PubSubClient TempControl("postman.cloudmqtt.com", 15449 , getdata, c);
...
void WiFiset()
{
  ...
  unsigned long int time_mqtt = millis();
  ...
  time_check = millis();
  if (WiFi.status() == WL_CONNECTED && millis() - time_mqtt < 5000)
  {
    while(!TempControl.connect("Temperature control", "vynhbdzh", "L_COV9WLalWk") && millis() - time_check < 6000)
    {
      Serial.print("/");
      delay(100);
    }
...
void stateMachine()
{
  switch(state)
  {
    ...
    case 1: //lock screen
      ...
      {
        TempControl.publish("unlock","Done");
        pass = 0;
        lcd.clear();
        state = 2;
      }
    ...
    case 2: //main menu - page 1 - temp/humid 
      ...
      {
        lcd.clear();
        TempControl.publish("unlock","Lock");
        pass = 0;
        state = 1;
      }
    ...
    case 18:
      ...
      if(cardStorage[posDelete] != "")
      {
        ...
        if (cardContent.substring(1) == "1C C5 DC 73")
        {
          ...
          deleteCard_ROM(posDelete*20 + 100);
          lcd.setCursor(0,0);
          lcd.print("Delete");
          lcd.setCursor(0,1);
          lcd.print("card");
